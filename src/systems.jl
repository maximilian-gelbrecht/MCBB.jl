...
struct stuart_landau_sathiyadevi_pars <: DEParameters
    lambda::Real
    omega::Real
    eps::Real
    N::Int
...
function stuart_landau_sathiyadevi_pars(lambda::Real, omega::Real, eps::Real, N::Int, P_1::Int, P_2::Int)
    P_1 = Int(round(r_1*N))
    P_2 = Int(round(r_2*N))

    A_1 = Bool.(adjacency_matrix(watts_strogatz(N, 2*P_1, 0)))
    A_2 = Bool.(adjacency_matrix(watts_strogatz(N, 2*P_2, 0)))

    stuart_landau_sathiyadevi_pars(lambda, omega, eps, N, P_1, P_2, A_1, A_2)
end

function stuart_landau_sathiyadevi_pars(lambda::Real, omega::Real, eps::Real, N::Int, r_1::Real, r_2::Real)
    P_1 = Int(round(r_1*N))
    P_2 = Int(round(r_2*N))
...
function stuart_landau_sathiyadevi!(dz,z,p::stuart_landau_sathiyadevi_pars,t)
    epsP1 = p.eps/(2*p.P_1)
    epsP2 = p.eps/(2*p.P_2)
